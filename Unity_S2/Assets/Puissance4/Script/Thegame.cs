...
public class Thegame : MonoBehaviourPun, IPunObservable
{
    ...
    [PunRPC]
    private void Support(int col, int row)
    {
        if (row != 5)
        {
    ...
    public void DropCol1()
    {
        ...
        else
        {
            ...
            if (row!= 5)
            {
                photonView.RPC("Support",RpcTarget.All,1,row);
            }
    ...
    public void DropCol2()
    {
        ...
        else
        {
            ...
            var spawn = new Vector3((float) 270.25, 25, 120); if (row!= 5)
            {
                photonView.RPC("Support",RpcTarget.All,2,row);
            }
    ...
    public void DropCol3()
    {
        ...
        else
        {
            ...
            if (row!= 5)
            {
                photonView.RPC("Support",RpcTarget.All,3,row);
            }
    ...
    public void DropCol4()
    {
        ...
        else
        {
            ...
            if (row!= 5)
            {
                photonView.RPC("Support",RpcTarget.All,4,row);
            }
    ...
    public void DropCol5()
    {
        ...
        else
        {
            ...
            if (row!= 5)
            {
                photonView.RPC("Support",RpcTarget.All,5,row);
            }
    ...
    public void DropCol6()
    {
        ...
        else
        {
            ...
            if (row!= 5)
            {
                photonView.RPC("Support",RpcTarget.All,6,row);
            }
    ...
    {
        bool  active = ((turn % 2 == 0 && PhotonNetwork.IsMasterClient) ||
                       (turn % 2 == 1 && (!PhotonNetwork.IsMasterClient)));
        stream.Serialize(ref listPlayer[0]);
        stream.Serialize(ref listPlayer[1]);
        ...
        if ( stream.IsWriting )
        {
            ...
            for (int i = 0; i < RowCount; i++)
            {
                for (int j = 0; j < ColsCount; j++)
                {
                    ...
                    {
                        stream.SendNext(1);
                        photonView.RPC("BoardPiece",RpcTarget.All,1, i,j);
                        
                        photonView.RPC("Support",RpcTarget.All,j,i);
                    }
                    else
                    {
                        stream.SendNext(2);
                        photonView.RPC("BoardPiece",RpcTarget.All,2, i,j);
    ...
    [PunRPC]
    public void BoardPiece(int Pion, int i , int j)
    {
        Board[i][j] = Pion.ToString()[0];
    }
